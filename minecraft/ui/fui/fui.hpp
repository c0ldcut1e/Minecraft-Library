...
namespace mc
{
    ...
    {
    public:
        static fui *GetInstance()
        {
            return MLink::DereferencePointerFromAddress<fui>(0x104F53A8);
        }

        void dispatchMouseMoveEvent(fuiFile *file, float x, float y)
        {
            MLINK_FUNC(void, 0x02BAAFF0, fui *, fuiFile *, float, float)(this, file, x, y);
        }

        fuiFile *getFuiFileFromIndex(uint32_t index)
        {
            return MLINK_FUNC(fuiFile *, 0x02BABABC, fui *, uint32_t)(this, index);
        }

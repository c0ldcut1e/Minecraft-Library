...
namespace mc
{
    ...
    {
    public:
        enum EJoypadButton : uint32_t
        {
            JOYPAD_BUTTON_B          = 0x1,
            JOYPAD_BUTTON_A          = 0x2,
            JOYPAD_BUTTON_Y          = 0x4,
            JOYPAD_BUTTON_X          = 0x8,
            JOYPAD_BUTTON_PLUS       = 0x10,
            JOYPAD_BUTTON_MINUS      = 0x20,
            JOYPAD_BUTTON_R          = 0x40,
            JOYPAD_BUTTON_L          = 0x80,
            JOYPAD_BUTTON_STICK_R    = 0x100,
            JOYPAD_BUTTON_STICK_L    = 0x200,
            JOYPAD_BUTTON_DPAD_UP    = 0x400,
            JOYPAD_BUTTON_DPAD_DOWN  = 0x800,
            JOYPAD_BUTTON_DPAD_LEFT  = 0x1000,
            JOYPAD_BUTTON_DPAD_RIGHT = 0x2000,
            JOYPAD_BUTTON_ZR         = 0x400000,
            JOYPAD_BUTTON_ZL         = 0x800000,
        };

        struct ButtonTapState
        {
            int count;
            uint64_t firstPressTime;
        };

        class C_4JInput
        {
        public:
            enum class EKeyboardMode
            {
                FULL,
                NUMPAD,
                UTF8,
        ...
        static bool ButtonCombo(uint32_t modifier, uint32_t button)
        {
            const CInput *input = CInput::GetInstance();
            return (input->buttonsHeld & modifier) != 0 && (input->buttonsPressed & button) != 0;
        }

        static bool ButtonPressed(uint32_t buttons)
        {
            return (CInput::GetInstance()->buttonsPressed & buttons) != 0;
        }

        static bool ButtonPressedTimes(uint32_t button, int presses, int windowFrames, CInput::ButtonTapState *state)
        {
            const CInput *input = CInput::GetInstance();
            if (((input->buttonsPressed & button) == 0) || ((input->buttonsHeld & MODIFIER_BUTTONS) != 0))
            {
                return false;
            }

            const uint64_t now    = System::processTimeInMilliSecs();
            const uint64_t window = static_cast<uint64_t>(windowFrames) * 1000 / 60;
            if (state->count == 0 || now - state->firstPressTime > window)
            {
                state->count          = 1;
        ...
        static bool ButtonsHeld(uint32_t buttons)
        {
            return (CInput::GetInstance()->buttonsHeld & buttons) == buttons;
        }
        ...
        {
            return MLINK_FUNC(VPADStatus *, 0x0340A24C, CInput *)(this);
        }

        bool RequestKeyboard(const wchar_t *param_1, const wchar_t *text, uint32_t param_3, uint32_t maxSize,
                             int (*callback)(void *data, bool okPressed), void *data, C_4JInput::EKeyboardMode keyboardMode)
        {
            return MLINK_FUNC(bool, 0x0340A33C, CInput *, const wchar_t *, const wchar_t *, uint32_t, uint32_t, int (*)(void *, bool), void *,
                              C_4JInput::EKeyboardMode)(this, param_1, text, param_3, maxSize, callback, data, keyboardMode);
        }

        static const uint32_t MODIFIER_BUTTONS =
                mc::CInput::JOYPAD_BUTTON_L | mc::CInput::JOYPAD_BUTTON_R | mc::CInput::JOYPAD_BUTTON_ZL | mc::CInput::JOYPAD_BUTTON_ZR;
        uint32_t field_0x0;
        uint32_t field_0x4;
        uint32_t field_0x8;
        uint32_t field_0xC;
        uint8_t vpadStatusRetrieved;
        uint8_t vpadStatusRetrievedLastPoll;
        uint8_t drcConnected;
        uint8_t drcDisconnectedThisFrame;
        uint8_t drcConnectedThisFrame;
        uint8_t field_0x15;
        uint8_t field_0x16;
        uint8_t field_0x17;
        uint32_t field_0x18;
        uint32_t buttonsHeld;
        uint32_t fakeButtons;
        uint32_t fakeInputTicksRemaining;
        uint32_t previousButtonsHeld;
        ...
        uint32_t field_0x7C;
        uint32_t field_0x80;
        uint8_t vpadStatusBufferIndex;
        uint8_t field_0x85;
        uint8_t field_0x86;
        uint8_t field_0x87;
        VPADStatus *vpadStatusBuffer;
        uint32_t field_0x8C;
        uint32_t field_0x90;
        uint32_t field_0x94;
        uint32_t field_0x98;
